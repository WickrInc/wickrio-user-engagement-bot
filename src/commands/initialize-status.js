...
class InitializeStatus {
  ...
  execute(messageService) {
    const currentEntries = this.statusService.initMessageEntries(messageService.getUserEmail());
    ...
    if (currentEntries.length < 1) {
      ...
      for (let i = 0; i < currentEntries.length; i += 1) {
        contentData = WickrIOAPI.cmdGetMessageIDEntry(currentEntries[i].message_id);
