...
const askStatus = function (messageService) {
  // const messageIdEntries = getMessageEntries(userEmail);
  currentMessageEntries = getMessageEntries(messageService.userEmail)
  logger.debug(`here are the messages:${currentMessageEntries}`)
  let reply = ''
  let obj
  if (currentMessageEntries.length < 1) {
    ...
    for (let i = 0; i < currentMessageEntries.length; i += 1) {
      contentData = WickrIOAPI.cmdGetMessageIDEntry(
        currentMessageEntries[i].message_id
      )
      const contentParsed = JSON.parse(contentData)
      messageList.push(contentParsed.message)
      messageString += `(${(index += 1)}) ${contentParsed.message}\n`
    }
    reply =
...
const messageChosen = function (messageService) {
  let reply
  let obj
  const index = messageService.message
  const length = Math.min(currentMessageEntries.length, 5)
  if (!isInt(index) || index < 1 || index > length) {
    ...
    const messageID = `${
      currentMessageEntries[parseInt(index, 10) - 1].message_id
    }`
